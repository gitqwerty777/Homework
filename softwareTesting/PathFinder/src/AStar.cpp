...
bool AStar::getPath(AStarNode* start, AStarNode* goal, std::vector<AStarNode*>& path)
{
	...
	std::make_heap(open.begin(), open.end(), CompareNodesAStar());
	pushOpen(start);

	while(!open.empty())
	{
		std::sort(open.begin(), open.end(), CompareNodesAStar());
...
{
	open.push_back(node);
	std::push_heap(open.begin(), open.end(), CompareNodesAStar());
	node->setOpen(true);
}

void AStar::popOpen(AStarNode* node)
{
	std::pop_heap(open.begin(), open.end(), CompareNodesAStar());
	open.pop_back();
	node->setOpen(false);
}
