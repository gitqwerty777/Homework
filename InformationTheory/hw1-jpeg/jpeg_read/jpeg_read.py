   ...
   for i in range(len(mcu)):
      ...
      for v in range(Vout):
         for h in range(Hout):
            for y in range(8):
               for x in range(8):
                  out[y+v*8][x+h*8].append(comp[(h//Hs)+Hin*(v//Vs)][y][x]) # has bug
   return out
...
def crop_image(data):
   ...
   for row in ([ [ data[y][x] for x in range(X) ] for y in range(Y) ]):
      print(row)
