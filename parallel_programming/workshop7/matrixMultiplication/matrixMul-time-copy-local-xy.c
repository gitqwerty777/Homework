...
int main(int argc, char *argv[])
{
  ...
  for (int device = 0; device < DEVICENUM; device++) {
    bufferC[device] = 
      clCreateBuffer(context, 
					 ...
					 ITEMPERDEVICE * sizeof(cl_uint), 
					 ((cl_uint *) tmp[0]) + device * ITEMPERDEVICE, 
					 &status);
    //assert(status == CL_SUCCESS);
  }
  ...
  printf("Kernel execution completes1.\n");
  ...
  for (int device = 0; device < DEVICENUM; device++) {
    bufferA[device] = 
      clCreateBuffer(context, 
					 CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
					 ITEMPERDEVICE * sizeof(cl_uint), 
					 ((cl_uint *)D) + device * ITEMPERDEVICE, 
					 &status);
    //assert(status == CL_SUCCESS);
  }
  /* bufferB */
  ...
  for (int device = 0; device < DEVICENUM; device++) {
    bufferC[device] = 
      clCreateBuffer(context, 
					 CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR,
					 ITEMPERDEVICE * sizeof(cl_uint), 
					 ((cl_uint *) F) + device * ITEMPERDEVICE, 
					 &status);
    //assert(status == CL_SUCCESS);
  }
  printf("Build buffers completes\n");
  ...
  /* waitforevent */
  clWaitForEvents(DEVICENUM, events); 
  printf("Kernel execution completes2.\n");  
