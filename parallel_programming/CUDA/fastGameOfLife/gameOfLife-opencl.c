...
__global__ void fillsur(__shared__ unsigned int sur[][BSIDE+2],  unsigned int * arr, int i, int j, int li, int lj, int n, int t){
  if(!(i <= n && j <= n)) return;  
    if(li == 1){
...
int main() {
  ...
  //配置裝置記憶體
  char* arr_device;
  cudaMalloc((void*) &arr_device, 2*MAXN*MAXN*sizeof(char));
  cudaMemcpy(arr_device, arr, 2*MAXN*MAXN*sizeof(char), cudaMemcpyHostToDevice);
  ...
  //下載裝置記憶體內容到主機上.
  cudaMemcpy(arr, arr_device, num*sizeof(Index), cudaMemcpyDeviceToHost);
