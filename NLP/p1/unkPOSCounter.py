...
if(syslen < 4 or sys.argv[1] != "-train" and sys.argv[1] != "-test"):
    print("ERROR: usage: trainPOSCounter.py [-train/-test] <tagfile> <featurefile> <label(optional)>")
    exit()
...
toption = sys.argv[1]
labellist = list()
labelfilename = "trainfeatures_mosthalf.txt"
if toption == "-train":
    flabel = open(labelfilename, "w")
    halflen = len(total_counter)/2
    for pair in total_counter.most_common(halflen):
