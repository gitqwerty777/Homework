...
def add_feature(sentence, index, label):
    ...
    # new feature : bigramscore
    if label:
        counteroffset = 1
    else:
        counteroffset = -1
    bigramScoreCounter[s1] = bigramScoreCounter[s1] + counteroffset
    bigramScoreCounter[s2] = bigramScoreCounter[s2] + counteroffset
    bigramScoreCounter[s3] = bigramScoreCounter[s3] + counteroffset
    bigramScoreCounter[s4] = bigramScoreCounter[s4] + counteroffset
...
for d in instances:
    ...
    bigramscore = 2147483647
    feateaureIndexs = Counter([label.index(d[1])+1, label.index(d[2])+1])
    bigramscore  = min(bigramscore, bigramScoreCounter[d[1]])
    bigramscore  = min(bigramscore, bigramScoreCounter[d[2]])

    if d[6] != "":
        feateaureIndexs[label.index(d[6])+1] = feateaureIndexs[label.index(d[6])+1] + 1
        bigramscore  = min(bigramscore, bigramScoreCounter[d[6]])
        bigramscore += bigramScoreCounter[d[6]]
    if d[7] != "":
        feateaureIndexs[label.index(d[7])+1] = feateaureIndexs[label.index(d[7])+1] + 1
        bigramscore  = min(bigramscore, bigramScoreCounter[d[7]])
