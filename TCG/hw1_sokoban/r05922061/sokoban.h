...
extern bool Get_Board(FILE *const fin, OldBoard *const b_ptr);
...
extern bool Inside(OldBoard const *const b_ptr, int const i, int const j);
...
extern bool Do_move(OldBoard *const b_ptr, Direction const dir, Vector_Move *const history);

// given the last move 'mv', undo the board '*b_ptr'
// the function asserts the validity of 'mv'
extern void Undo(OldBoard *const b_ptr, Move const mv);
