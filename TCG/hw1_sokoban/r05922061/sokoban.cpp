...
bool Get_Board(FILE *const fin, OldBoard *const b_ptr){
...
#define SOKOBAN_VM_ALLOC do{\
    if(history->size == history->capacity){\
        if(history->capacity == 0){\
            history->data = (Move*)malloc(sizeof(Move));     \
            history->capacity = 1;\
        }else{\
            history->data = (Move*)realloc(history->data, 2*history->capacity*sizeof(Move)); \
            history->capacity *= 2;\
        }\
    }\
}while(0)
...
int const Di[]={0, 0, -1, 1}, Dj[]={-1, 1, 0, 0};
bool Inside(OldBoard const *const b_ptr, int const i, int const j){//two const!!!, int const and const int diff
    return 0<=i && i<=b_ptr->height-1 && 0<=j && j<=b_ptr->width-1 && b_ptr->board[i][j]!='#';
}
bool Do_move(OldBoard *const b_ptr, Direction const dir, Vector_Move *const history){
...
void Undo(OldBoard *const b_ptr, Move const mv){
